...
## 🔴 GRAQLE GOVERNANCE CONSTITUTION (MANDATORY — overrides default behaviour)

> This section is enforced two ways: the `.claude/settings.json` permission
> wall (native `Edit`/`Write`/`Bash` are **denied**) and the
> `.claude/hooks/graqle-gate.py` PreToolUse hook (fail-closed backstop).
> Treat it as binding, not advisory. Remove `.claude/hooks/graqle-gate.py`
> to disable.

**Prime directive:** All code changes go through the GraQle tool chain.
The AI writes ZERO code with native tools — only via `graq_generate` /
`graq_edit`. Native `Edit`/`Write`/`Bash` are blocked by design.

### 9-Phase Workflow (apply by task type)

| Phase | Tool(s) | When |
|-------|---------|------|
| 1 — SESSION BOOT | `graq_lifecycle(session_start)` → `graq_reload()` → `graq_lessons` | Once per session |
...
| 4 — PLAN | `graq_plan(goal, scope)` — also unlocks `graq_bash` | Per change |
...
| 7 — SENTINEL REVIEW | `graq_preflight` → `graq_review` ×2 → `graq_predict` → `graq_review(focus="security")` | Per PR |
...
**Phase selection:** read-only investigation = phases 1–2 only; single-file
fix = 1→2→3→4→5→6→7→9; new feature / multi-file / refactor = all 9.

### Non-negotiable rules

1. **Session boot (Phase 1) before any other tool call.**
2. **No phase skipping.** Log any forced skip with a reason.
3. **`graq_review` BEFORE and AFTER implementation** — implement-first-review-later is banned.
4. **Low GraQle confidence → diagnose and re-ask properly. NEVER bypass** to manual reasoning. (A bad question, not a bad graph, is the usual cause.)
5. **CRITICAL blast radius → stop and escalate to the user** before any change.
6. **Phase 9 (`graq_learn`) is mandatory** — the graph must grow from every fix.

### Cost-optimised routing

- `graq_safety_check` replaces impact+preflight+reason when risk is unknown (3 calls → 1).
- `graq_review(focus="security")` on the final pass only; full `focus="all"` is multi-agent.
- `graq_reason` only when `graq_safety_check` returns MEDIUM/HIGH risk.
- `graq_plan(dry_run=true)` cheaply unlocks `graq_bash` (CG-02) without a full DAG.
- One PR per logical group of related changes (fewer sentinel chains = lower cost).

### Known workarounds

- `graq_bash` blocked until a plan exists → `graq_plan(dry_run=true)` first.
- `graq_write` rejects existing files → use `graq_edit(strategy="literal")`.
- PR `--body` inline truncates → write to `/tmp/pr_body.md`, use `--body-file`.
...
# ── Constitution single source of truth (package data) ──────────────────
# The full governed-development rulebook lives in graqle/data/constitution/
# as ordered .md fragments. We load + concatenate them so a single edit
# updates every client (CLAUDE.md / AGENTS.md / .cursorrules / ...). If the
# package data is missing (e.g. a stripped wheel), we fall back to the inline
# _FALLBACK_INSTRUCTIONS above so `graq init` never breaks.

# Fragments are concatenated in sorted filename order. The NN- numeric prefix
# defines that order; new fragments slot in by choosing a prefix.
_CONSTITUTION_PACKAGE = "graqle.data.constitution"


def _load_constitution() -> str:
    """Return the assembled constitution text from package data.

    Reads every ``*.md`` fragment under ``graqle/data/constitution/`` in
    sorted filename order and joins them. On ANY failure (package missing,
    no fragments, read error) returns ``_FALLBACK_INSTRUCTIONS`` so init
    is never blocked by a packaging issue.
    """
    try:
        from importlib import resources

        root = resources.files(_CONSTITUTION_PACKAGE)
        fragments = sorted(
        ...
        for frag in fragments:
            try:
                texts.append(frag.read_text(encoding="utf-8").strip())
        ...
        if assembled.strip():
            return assembled
        logger.debug(
            "constitution: no fragments found under %s; using fallback",
            _CONSTITUTION_PACKAGE,
        )
        ...
        # Specific exceptions only — never swallow SystemExit/KeyboardInterrupt.
        # A packaging issue must degrade to the inline fallback, never break
        # `graq init`, but leave a debug breadcrumb for troubleshooting.
        logger.debug("constitution load failed (%s); using fallback", exc)
        return _FALLBACK_INSTRUCTIONS


# Public name used throughout init.py. Loaded once at import.
AI_INSTRUCTIONS_SECTION = _load_constitution()
