"""AWS Lambda handler for CogniGraph API.
...
Environment variables:
  COGNIGRAPH_GRAPH_PATH   — S3 URI or local path to cognigraph.json
  COGNIGRAPH_CONFIG_PATH  — S3 URI or local path to cognigraph.yaml
...
logger = logging.getLogger("cognigraph.lambda")
...
def _ensure_graph_local() -> tuple[str, str]:
    ...
    graph_path = os.environ.get("COGNIGRAPH_GRAPH_PATH", "cognigraph.json")
    config_path = os.environ.get("COGNIGRAPH_CONFIG_PATH", "cognigraph.yaml")

    if graph_path.startswith("s3://"):
        local_graph = os.path.join(tmp, "cognigraph.json")
    ...
    if config_path.startswith("s3://"):
        local_config = os.path.join(tmp, "cognigraph.yaml")
...
def _get_app():
    ...
    from cognigraph.server.app import create_app
