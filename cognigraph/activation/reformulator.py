...
ADR-104: When CogniGraph runs inside an AI tool (Claude Code, Cursor, Codex),
...
Architecture:
    1. Auto-detect: Is CogniGraph running inside an AI tool? (env vars)
    2. If yes: reformulate using the chat context (zero extra model calls —
       the AI tool does the reformulation before calling CogniGraph)
...
logger = logging.getLogger("cognigraph.activation.reformulator")
...
class ReformulationContext:
    ...
    When CogniGraph is invoked from Claude Code / Cursor / Codex, the AI tool
    ...
    Attributes:
        ...
        attachments: Screenshots, files, or other media the user attached.
                     ...
                     passing to CogniGraph — we only use the text descriptions.
    """
...
class QueryReformulator:
    """Context-aware query reformulator for CogniGraph.
    ...
    def _reformulate_with_context(
        ...
        # CogniGraph can activate the right nodes.
