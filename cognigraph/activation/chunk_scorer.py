...
.cognigraph/chunk_embeddings.npz. At query time only the query is
...
class ChunkScorer:
    ...
    v0.12.3: Uses precomputed embedding cache (.cognigraph/chunk_embeddings.npz)
    ...
    def _load_cache(self, graph: Any) -> bool:
        """Try loading precomputed chunk embeddings from .cognigraph/chunk_embeddings.npz."""
        if self._cache_loaded:
            return self._chunk_matrix is not None

        self._cache_loaded = True
        cache_path = Path(".cognigraph/chunk_embeddings.npz")
    ...
    def build_cache(self, graph: Any) -> None:
        """Precompute embeddings for all chunks and save to .cognigraph/chunk_embeddings.npz.

        Called by `kogni rebuild --embeddings` or `kogni init`.
        """
        chunk_keys: list[str] = []
