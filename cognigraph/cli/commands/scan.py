...
class RepoScanner:
    ...
    def _resolve_imports(self) -> None:
        ...
        for node_id in self._py_files:
            # e.g., "cognigraph/core/graph.py" -> "cognigraph.core.graph"
        ...
        for node_id, path in self._py_files.items():
            ...
            for imp in info["imports"]:
                ...
                if target is None:
                    # Try sub-module: "cognigraph.core" matches "cognigraph/core/__init__.py"
...
scan_app = typer.Typer(help="Scan a codebase to build a CogniGraph knowledge graph.")


@scan_app.command("repo")
def scan_repo(
    path: str = typer.Argument(".", help="Repository path to scan"),
    output: str = typer.Option("cognigraph.json", "--output", "-o", help="Output JSON file path"),
    ...
    Output is networkx JSON format, compatible with ``CogniGraph.from_json()``.
    """
    if verbose:
    ...
    console.print(Panel(
        f"[bold cyan]CogniGraph Repository Scanner[/bold cyan]\n"
        f"Path: {repo}\n"
