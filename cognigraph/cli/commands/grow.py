"""kogni grow — incrementally update the knowledge graph.

Called automatically by the git post-commit hook. Can also be run manually.
This is the core of CogniGraph's promise: the graph adapts and grows with
...
The grow command:
  ...
  3. Merges into existing cognigraph.json
  4. Updates metrics
  5. Runs in <2 seconds for typical commits

Usage:
    kogni grow             # Interactive output
    kogni grow --quiet     # Silent (for git hooks)
    kogni grow --full      # Full rescan (not just diff)
"""
...
console = Console()
logger = logging.getLogger("cognigraph.cli.grow")
...
def grow_command(
    ...
    re-ingests markdown KG sources, and merges into cognigraph.json.
    ...
    graph_path = root / "cognigraph.json"
    if not graph_path.exists():
        if not quiet:
            console.print("[yellow]No cognigraph.json found. Run 'kogni init' first.[/yellow]")
        return
    ...
    try:
        ...
        if not quiet:
            console.print("[red]Failed to read cognigraph.json[/red]")
        return
    ...
    if full:
        # Full rescan
        try:
            from cognigraph.cli.commands.init import scan_repository
    ...
    from cognigraph.cli.commands.ingest import _merge_graphs
