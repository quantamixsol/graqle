"""kogni link — Multi-project graph operations.

Merge multiple project KGs into a unified graph and create cross-project
edges. This enables CogniGraph reasoning across your entire ecosystem.

Examples:
    kogni link merge project1/cognigraph.json project2/cognigraph.json -o merged.json
    kogni link edge crawlq/tamr_sdk frictionmelt/retrieval --relation POWERS
    kogni link stats merged.json
"""
...
def link_merge(
    ...
    \b
    Examples:
        kogni link merge project1/cognigraph.json project2/cognigraph.json
        kogni link merge *.json --output unified.json --no-prefix
    """
    if len(graphs) < 2:
    ...
    for gpath in graphs:
        ...
        if project_name in (".", ""):
            project_name = p.stem.replace("cognigraph", "default")
...
def link_edge(
    ...
    Examples:
        kogni link edge crawlq/tamr_sdk frictionmelt/retrieval --relation POWERS
        kogni link edge cognigraph/reasoning tracegov/compliance --relation ENABLES
    """
    ...
    graph = CogniGraph.from_json(str(p), config=config)
...
def link_stats(
    graph_path: str = typer.Argument("cognigraph.json", help="Path to merged graph file"),
) -> None:
