"""kogni init — set up intelligent development for any project and IDE.
...
and .kogni/ workspace structure. Works with any IDE or terminal.
"""
...
# CogniGraph — Dev Intelligence Layer

> Auto-generated by `kogni init`. Provides your AI coding assistant with
...
CogniGraph turns your codebase into a **knowledge graph** where every module,
...
|---------|-------------|
| `kogni run "<query>"` | Reason over the knowledge graph |
| `kogni context <name>` | Get 500-token focused context for any entity |
| `kogni inspect --stats` | Graph statistics (nodes, edges, density) |
| `kogni scan repo .` | Re-scan codebase and rebuild the KG |
| `kogni doctor` | Health check your installation |
| `kogni setup-guide` | Step-by-step backend setup |

### MCP Tools (Claude Code, Cursor, VS Code, and MCP-compatible IDEs)
| Tool | Purpose |
|------|---------|
| `kogni_context` | Focused context for a service/module |
| `kogni_reason` | Multi-agent graph reasoning |
| `kogni_inspect` | Graph structure inspection |
| `kogni_preflight` | Pre-change safety check |
| `kogni_impact` | Impact analysis for changes |
| `kogni_lessons` | Past mistake patterns |
...
## Session Continuity (optional — .kogni/ workspace)

CogniGraph can maintain structured memory across sessions:
...
### Session Start
1. Read `.kogni/main.md` (~300 tokens)
2. Read `.kogni/branches/{active}/commit.md` (~400 tokens)
...
- `cognigraph.yaml` — Model backend, cost budgets, activation strategy
- `cognigraph.json` — The knowledge graph (auto-generated by `kogni scan repo .`)
- `.mcp.json` — MCP server registration (IDE-specific)

To re-scan: `kogni scan repo .`
To query: `kogni run "your question"`
To get context: `kogni context <service-name>`
""".strip()
...
SUPPORTED_IDES = {
    ...
    "generic": "Any IDE / terminal (kogni CLI only, no MCP)",
}
...
def _get_instructions_marker(ide: str) -> str:
    ...
    return "# CogniGraph — Dev Intelligence Layer"
...
# ──────────────────────────────────────────────────────────────────────
# /kogni skill — smart query router for Claude Code
# ──────────────────────────────────────────────────────────────────────

KOGNI_SKILL_CONTENT = r"""Smart query router for project knowledge. Classifies the user's question and picks the cheapest approach that gives a correct answer. NEVER default to CogniGraph for simple lookups.
...
```
1. Grep for the key term in .gcc/project-kg.md (or cognigraph.json)
...
**DO NOT** use CogniGraph MCP tools for lookups. They are 5-10x more expensive than a grep.
...
**Use CogniGraph only if** project-kg.md doesn't have the relationships needed.

#### IMPACT -> kogni_impact MCP tool (cost: ~800-1500 tokens)
```
1. Call mcp__cognigraph__kogni_impact with the component name
2. Present the impact trace to the user
```
**This is where CogniGraph earns its keep** — traversing dependency chains automatically.

#### REASONING -> kogni_reason MCP tool (cost: ~1500-3000 tokens)
```
1. Call mcp__cognigraph__kogni_reason with the question, max_rounds=2
2. If confidence < 0.5, supplement with targeted file reads
3. Present synthesized answer
```
**This is CogniGraph's sweet spot** — multi-hop reasoning across nodes.

#### PREFLIGHT -> kogni_preflight MCP tool (cost: ~500-1000 tokens)
```
1. Call mcp__cognigraph__kogni_preflight with the action and files
2. Present warnings, lessons, and safety boundaries
```

#### LESSONS -> kogni_lessons MCP tool (cost: ~400-800 tokens)
```
1. Call mcp__cognigraph__kogni_lessons with the operation
...
- `[KOGNI: IMPACT -> kogni_impact | ~1200 tokens]`
- `[KOGNI: REASONING -> kogni_reason (3 rounds) | ~2500 tokens]`

### Step 4: Answer the question

After routing, answer the user's question using the chosen approach. Be concise. Cite the source (file:line or CogniGraph node IDs).
...
1. **Cheapest first.** Always try grep before CogniGraph.
2. **Never use kogni_reason for lookups.** It's 10x the cost of a grep.
...
def _verify_backend(
    ...
    if resolved_key is None:
        ...
        console.print(
            ...
            f"  queries (kogni run, kogni_reason) will fail until you set it.\n"
        )
        console.print(
    ...
    try:
        if backend == "anthropic":
            ...
            console.print(
                ...
                f"  [dim]Test manually: kogni run \"hello\"[/dim]\n"
            )
...
# ──────────────────────────────────────────────────────────────────────
# /kogni skill installer
# ──────────────────────────────────────────────────────────────────────


def _install_kogni_skill(root: Path) -> bool:
    """Install the /kogni smart-routing skill for Claude Code.

    Installs to TWO locations for maximum coverage:
    1. Project-local: {root}/.claude/commands/kogni.md  (for this project)
    2. User-global:   ~/.claude/commands/kogni.md       (for all projects)
    ...
    local_path = local_dir / "kogni.md"
    if local_path.exists():
        existing = local_path.read_text(encoding="utf-8", errors="ignore")
        if "Smart query router" in existing:
            console.print("  [dim].claude/commands/kogni.md already installed — skipping[/dim]")
        else:
            local_dir.mkdir(parents=True, exist_ok=True)
            local_path.write_text(KOGNI_SKILL_CONTENT, encoding="utf-8")
            console.print("  [green]+[/green] .claude/commands/kogni.md (project skill)")
            installed = True
    else:
        local_dir.mkdir(parents=True, exist_ok=True)
        local_path.write_text(KOGNI_SKILL_CONTENT, encoding="utf-8")
    ...
    global_path = global_dir / "kogni.md"
    if global_path.exists():
        ...
        if "Smart query router" in existing:
            ...
            console.print("  [green]+[/green] ~/.claude/commands/kogni.md (global skill)")
            installed = True
    else:
        global_dir.mkdir(parents=True, exist_ok=True)
        global_path.write_text(KOGNI_SKILL_CONTENT, encoding="utf-8")
    ...
    if target.exists():
        try:
            ...
            if "cognigraph" in existing.get("mcpServers", {}):
                console.print(f"  [dim]{target.name} already has cognigraph entry — skipping[/dim]")
                return False
            existing["mcpServers"]["cognigraph"] = mcp_entry["mcpServers"]["cognigraph"]
            target.write_text(json.dumps(existing, indent=2), encoding="utf-8")
            console.print(f"  [green]Merged cognigraph into existing {target.name}[/green]")
            return True
...
def _write_claude_md(root: Path, ide: str = "claude") -> bool:
    ...
    old_marker = "# CogniGraph — Governed Development Protocol"
    ...
    if target.exists():
        ...
        if marker in existing or old_marker in existing:
            console.print(f"  [dim]{target.name} already has CogniGraph section — skipping[/dim]")
            return False
        # Append
        with open(target, "a", encoding="utf-8") as f:
            f.write("\n\n" + AI_INSTRUCTIONS_SECTION + "\n")
        console.print(f"  [green]Appended CogniGraph section to existing {target.name}[/green]")
        return True
...
def _write_gcc_structure(root: Path) -> bool:
    """Create .kogni/ workspace structure. Skips if it already exists.

    Also supports legacy .gcc/ — if that exists, we skip to avoid confusion.
    """
    # Support legacy .gcc/ and new .kogni/
    legacy = root / ".gcc"
    kogni_dir = root / ".kogni"

    if legacy.exists() or kogni_dir.exists():
        name = ".gcc" if legacy.exists() else ".kogni"
        console.print(f"  [dim]{name}/ already exists — skipping[/dim]")
        return False

    # Create directories using new .kogni/ name
    (kogni_dir / "branches" / "main").mkdir(parents=True, exist_ok=True)
    (kogni_dir / "checkpoints").mkdir(parents=True, exist_ok=True)

    # Write files
    (kogni_dir / "main.md").write_text(_build_gcc_main_md(root), encoding="utf-8")
    (kogni_dir / "registry.md").write_text(_build_gcc_registry_md(), encoding="utf-8")
    (kogni_dir / "config.yaml").write_text(_build_gcc_config_yaml(), encoding="utf-8")
    (kogni_dir / "branches" / "main" / "commit.md").write_text(
        "# Commit Log — main\n\n", encoding="utf-8"
    )
    (kogni_dir / "branches" / "main" / "log.md").write_text(
        "# Session Log — main\n\n", encoding="utf-8"
    )
    (kogni_dir / "branches" / "main" / "metadata.yaml").write_text(
        _build_gcc_metadata_yaml(), encoding="utf-8"
    )
    # .gitkeep for checkpoints
    (kogni_dir / "checkpoints" / ".gitkeep").write_text("", encoding="utf-8")
...
def init_command(
    ...
    no_skill: bool = typer.Option(
        False, "--no-skill", help="Skip /kogni skill installation"
    ),
    ...
    """Initialize CogniGraph for your project and IDE.
    ...
    \b
    Specific IDE:
        kogni init --ide cursor
        kogni init --ide vscode
        kogni init --ide generic    (CLI only, no MCP)
    ...
    console.print(
        Panel(
            "[bold cyan]CogniGraph[/bold cyan] — Dev Intelligence Layer\n"
    ...
    # /kogni skill (smart query router)
    if not no_skill:
        _install_kogni_skill(root)

    # .kogni/ workspace
    if not no_gcc:
        created = _write_gcc_structure(root)
        if created:
            console.print(f"  [green]+[/green] .kogni/ (session workspace)")
    ...
    console.print(
        Panel(
            ...
            "  1. [bold]kogni doctor[/bold]          — verify your setup is complete\n"
            "  2. [bold]/kogni <question>[/bold]     — smart-routed query (cheapest approach)\n"
            "  3. [bold]kogni run \"query\"[/bold]    — full graph reasoning\n\n"
            "[bold]All commands:[/bold]\n"
            "  [bold]kogni doctor[/bold]             — health check (run this first!)\n"
            "  [bold]kogni context <name>[/bold]    — 500-token focused context\n"
            "  [bold]kogni ingest[/bold]             — re-ingest knowledge sources\n"
            "  [bold]kogni metrics[/bold]            — usage metrics and ROI\n"
