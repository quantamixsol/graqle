"""Core metrics tracking engine for CogniGraph.

Persists usage data to `.cognigraph/metrics.json` and provides ROI
...
class MetricsEngine:
    """Track and report CogniGraph usage metrics.
    ...
    def __init__(self, metrics_dir: Path | None = None) -> None:
        if metrics_dir is None:
            metrics_dir = Path.cwd() / ".cognigraph"
    ...
    def record_context_load(
        ...
        tokens_returned:
            Actual tokens returned by CogniGraph.
        tokens_without:
            Estimated tokens that would have been loaded without CogniGraph
